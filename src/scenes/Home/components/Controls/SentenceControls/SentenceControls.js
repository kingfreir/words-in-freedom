...
class SentenceControls extends Component {
  ...
  componentWillReceiveProps({ sentenceRef }) {
    if (sentenceRef !== this.props.sentenceRef) {
      this.setState({
        value: sentenceRef.state.sentence || sentenceRef.props.sentence
      });
    }
  }
  ...
  handleSentenceChange = ev => {
    ...
    this.setState({ value });
    sentenceRef.setSentenceParameter(
      { sentence: value },
  ...
  render() {
    const { sentenceRef, color } = this.props;

    const { props, state } = sentenceRef;
